...
public class StatementOfAssetsViewer
{
    ...
    private Control createColumns(Composite parent, boolean isConfigurable) // NOSONAR
    {
        ...
        column.setMenuLabel(
                        Messages.ColumnProfitLoss + " (" + Messages.LabelCapitalGainsMethodFIFO + ")"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        column.setGroupLabel(Messages.ColumnProfitLoss);
        column.setMenuLabel(
                        Messages.ColumnProfitLoss + " (" + Messages.LabelCapitalGainsMethodMovingAverage + ")"); //$NON-NLS-1$ //$NON-NLS-2$
        labelProvider = new ReportingPeriodLabelProvider(
                        new ElementValueProvider(LazySecurityPerformanceRecord::getCapitalGainsOnHoldingsMovingAverage,
                                        withSum()),
                        true);
        column.setLabelProvider(labelProvider);
        column.setSorter(ColumnViewerSorter.create(new ElementComparator(labelProvider)));
